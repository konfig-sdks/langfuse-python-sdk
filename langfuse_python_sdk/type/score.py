# coding: utf-8

"""
    langfuse

    ## Authentication  Authenticate with the API using Basic Auth, get API keys in the project settings:  - username: Langfuse Public Key - password: Langfuse Secret Key

    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from langfuse_python_sdk.type.score_source import ScoreSource

class RequiredScore(TypedDict):
    id: str

    traceId: str

    name: str

    value: typing.Union[int, float]

    source: ScoreSource

    timestamp: datetime

class OptionalScore(TypedDict, total=False):
    observationId: typing.Optional[str]

    comment: typing.Optional[str]

class Score(RequiredScore, OptionalScore):
    pass
